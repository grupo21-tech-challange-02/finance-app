...
const transactionSchema = z
  .object({
    type: z
      .enum(TYPE_VALUES)
      .refine((val) => TYPE_VALUES.includes(val), "Selecione o tipo de transação."),
    category: z
      .enum(CATEGORY_VALUES)
      .refine((val) => CATEGORY_VALUES.includes(val), "Selecione uma categoria."),
...
export default function TransactionForm({ initial, onSaved, onCancel }: Props) {
  ...
  const valueBadge = (
    <span
      className={`inline-flex h-6 w-6 items-center justify-center rounded-md text-xs font-bold ${watchedType === "saque"
